...
詳細な手順は [@docs/release.md](../../docs/release.md) を参照。

`charts/**` を `main` に push すると `helm-release.yml` が自動実行され、GitHub Release と `gh-pages` の `index.yaml` が更新される。**必ず** `Chart.yaml` の `version` を上げること（上げないと chart-releaser がスキップする）。
